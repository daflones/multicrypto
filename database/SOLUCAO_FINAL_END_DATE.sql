-- =====================================================
-- SOLUÇÃO FINAL: ATUALIZAÇÃO DE DATA DE TÉRMINO (END_DATE)
-- =====================================================

-- 1. Função de Progresso (Base para o limite de 300%)
...
-- 2. Função Principal: Processa Comissão e Reduz Data
...
BEGIN
  ...
  -- DISTRIBUIÇÃO E REDUÇÃO DE TEMPO
  ...
  IF v_total_remaining > 0 THEN
    ...
    LOOP
      ...
      IF v_daily_yield_val > 0 AND v_investment.end_date IS NOT NULL THEN
        -- ⏰ AQUI ESTÁ A MÁGICA: REDUÇÃO DE TEMPO
        -- Calcula tempo exato que a comissão representa
        v_reduction_interval := (v_amount_to_add / v_daily_yield_val) * INTERVAL '1 day';
        
        -- Subtrai esse tempo da data final atual
        v_new_end_date := v_investment.end_date - v_reduction_interval;
        
        -- Se a nova data caiu no passado (já deveria ter acabado), ajusta para AGORA
        ...
        RAISE NOTICE 'Investimento %: Reduzido em % (Nova data: %)', v_investment.id, v_reduction_interval, v_new_end_date;
      ELSE
        UPDATE public.user_investments
        SET total_earned = v_new_total_earned
        WHERE id = v_investment.id;
